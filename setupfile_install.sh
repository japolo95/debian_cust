...
yes 0000 | su -c "echo -e '\nuser ALL=(ALL:ALL) NOPASSWD:ALL' >> /target/etc/sudoers"

#PART 0.1: add sbin paths to environment
echo "creating sbin paths"
sudo echo -e "\nexport PATH=$PATH:/sbin" >> /target/home/user/.bash_profile #alternatively ~/.bash_profile
sudo echo -e "\nexport PATH=$PATH:/sbin" >> /target/home/user/.bashrc #alternatively ~/.bashrc
...
su -c "echo 'workmachine' > /target/etc/hostname" #sudo would not work, the arrow symbol has privilegies of current shell, not sudo

#PART 1: remove cd from resrouces and add resources for apps installation
#========================================================================

echo "Updating resources"
sudo sed -i '/cdrom/d' /target/etc/apt/sources.list
sudo echo -e "\ndeb http://deb.debian.org/debian bullseye main\ndeb-src http://deb.debian.org/debian/ bullseye main\n\ndeb http://security.debian.org/debian-security bullseye-security main contrib\ndeb-src http://security.debian.org/debian-security bullseye-security main contrib\n\ndeb http://deb.debian.org/debian/ bullseye-updates main contrib\ndeb-src http://deb.debian.org/debian/ bullseye-updates main contrib" > /target/etc/apt/sources.list
